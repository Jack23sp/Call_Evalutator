...
for( var i = 0, len = k.getTotal(); i < len; i++ ) {
	...
	bullet.onclick = function (event) { k.show(event.target.getAttribute('index')); return false; };
	bullet.ontouchstart = function (event) { k.show(event.target.getAttribute('index')); return false; };
	bullet.id = "bullet_child" + i;
	bulletsContainer.appendChild(bullet);
}

// Update the bullets when the layer changes
k.changed.add( function( layer, index ) {
	var bullets = document.body.querySelectorAll( '.bullets button' );
